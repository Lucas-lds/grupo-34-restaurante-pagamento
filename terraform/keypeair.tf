...
resource "aws_secretsmanager_secret" "pagamento" {
  name = "keypair-ms-pagamento-v1" # Nome do segredo que será armazenado
}

# Recurso para armazenar a versão do segredo no AWS Secrets Manager
resource "aws_secretsmanager_secret_version" "pagamento" {
  secret_id = aws_secretsmanager_secret.pagamento.id # ID do segredo que estamos atualizando
